...
$branch = array();
 $branches = $conn->query("SELECT * FROM branches where id in (SELECT from_branch_id FROM parcels where id in ($ids) ) or id in (SELECT to_branch_id FROM parcels where id in ($ids) ) ");
